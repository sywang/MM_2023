...
def split_adata(adata: ad.AnnData, split_by_obs_column: Optional[str] = None, train_col_name: str = "train") ->\
        (ad.AnnData, ad.AnnData):
    if split_by_obs_column is None:
        train_obs_index, test_obs_index = train_test_split(adata.obs.index)
        return adata[train_obs_index], adata[test_obs_index]
    else:
        specific_columns_values = set(adata.obs[split_by_obs_column].values)
        train_values, test_values = train_test_split(list(specific_columns_values))
